function varargout = movie_spacetime_stimonly(varargin)
% MOVIE_SPACETIME_STIMONLY Application M-file for songwave_specgram.fig
%    FIG = MOVIE_SPACETIME_STIMONLY launch songwave_specgram GUI.
%    MOVIE_SPACETIME_STIMONLY('callback_name', ...) invoke the named callback.
...
if nargin == 0  % LAUNCH GUI
    ...
    global rawStimData rawStimDS sDim
    if rawStimData == 0
    ...
    if ~isempty(sDim)
        if sDim == '1-D'
            tt=errordlg('This option is only for 2-D files.',...
                'Data Selection Error','modal');
            uiwait(tt);
            delete(handles.figure1);
            return;
...
for ii=1:numfiles
    ...
    switch ext
        ...
        case {'.mat'}  % 2-D raw data (movie)
            stimstim = load(rawStimDS{ii}.stimfiles);
        ...
        case {'.dat', '.txt'}
            stimstim = dlmread(rawStimDS{ii}.stimfiles);
    ...
    % Take linear transformation of movie
    stim=reshape(stimstim, xsize*ysize, framesize);
    [NBAND, StimDS{ii}.nlen] = size(stim);

    save(fullfile(outputPath,[name,'_Stim_',num2str(ii),'.mat']), 'stim');
    % Assign values to global variable StimDS
    StimDS{ii}.stimfiles = fullfile(outputPath,[name,'_Stim_',num2str(ii),'.mat']);
    %StimDS{ii}.nlen = framesize;
    ...
    %
%     [rpath,rname,rext,rver] = fileparts(rawStimDS{ii}.respfiles);
%     switch rext
%     case {'.dat', '.txt', ''}
%         newpsth = load(rawStimDS{ii}.respfiles);
% 
%     case {'.mat'}
%         respMat = load(rawStimDS{ii}.respfiles);
% 
...
%             if iscell(rawResp)
%                 spiketrain = zeros(rawStimDS{ii}.ntrials,rawStimDS{ii}.nlen);
%                 for trial_ind =1:rawStimDS{ii}.ntrials
% 
...
% 
%     % Assign values to global variable StimDS
%     StimDS{ii}.respfiles = fullfile(outputPath,[name,'_Spike_time_',num2str(ii),'.mat']);
%     StimDS{ii}.ntrials = size(newpsth, 1);
...
function varargout = reset_Callback(h, eventdata, handles, varargin)
    ...
    %clear StimDS initialFreq endFreq StimSampRate sDim
    clear global StimDS NBAND num_trials  sDim
    ...
    set(handles.inprogress, 'visible', 'off')
    set(handles.completed, 'visible', 'off')
    set(handles.waitaxes, 'visible', 'off');
    child = get(handles.waitaxes, 'Children');
    set(child, 'visible', 'off');
...
function displayspecgram(handles)
    ...
    forward = Check_And_Load(StimDS{handles.index}.stimfiles);
%     newpsth = Check_And_Load(StimDS{handles.index}.respfiles);
%     newpsth(find(newpsth < 0)) = 0;
