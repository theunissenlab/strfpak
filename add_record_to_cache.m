function add_record_to_cache(cached_dir,the_checksum,time_saved,function_name,data_filename);
...
outfile = promote_cache(the_checksum,cached_dir);  %If it doesn't exist, creates a file to keep track of how useful this calculation is.
save(outfile,'time_saved','function_name','-APPEND');
load(outfile);
save(data_filename,'time_saved','function_name','space_needed','-APPEND');

%  Old version:
%function active = add_record_to_cache(cached_dir,maxsize,the_checksum,time_saved,space_needed);
%
