function varargout = preprocessmenu_pred(varargin)
% PREPROCESSMENU_PRED Application M-file for preprocessmenu_pred.fig
%    FIG = PREPROCESSMENU_PRED launch preprocessmenu_pred GUI.
%    PREPROCESSMENU_PRED('callback_name', ...) invoke the named callback.
...
global preprocessOption_pred

tt=songwave_spectrum_pred;
uiwait(tt);
preprocessOption_pred = 'SongWave->Spectrogram(STFFT)';
...
tt=wavelet1d_scalogram_pred;
uiwait(tt);
preprocessOption_pred = 'SongWave->Scalogram(wavelet)';
delete(handles.figure1);
strfFirstGUI;
% --------------------------------------------------------------------
...
tt=movie_pfft_pred;
uiwait(tt);
preprocessOption_pred = 'Fourier Power transform';
delete(handles.figure1);
strfFirstGUI;
...
tt=movie_spacetime_pred;
uiwait(tt);
preprocessOption_pred = 'Space-Time transform';
delete(handles.figure1);
strfFirstGUI;
